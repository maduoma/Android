package com.dodemy.room_todo.fragments
...
class AddToDoDialog : DialogFragment()
{
    ...
    private fun populateForm(toDo: ToDo)
    {
        ...
        when(toDo.priority)
        {
            Priorities.LOW.name -> binding.srPriority.setSelection(0)
            Priorities.MEDIUM.name -> binding.srPriority.setSelection(1)
            Priorities.HIGH.name -> binding.srPriority.setSelection(2)
        }
    ...
    private fun createToDo() = ToDo(
        ...
        {
            0L -> Priorities.LOW.name
            1L -> Priorities.MEDIUM.name
            else -> Priorities.HIGH.name
        }
    )
}
