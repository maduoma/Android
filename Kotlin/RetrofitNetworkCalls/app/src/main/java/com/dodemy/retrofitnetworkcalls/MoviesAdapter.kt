...
class MoviesViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
    ...
    fun bind(movie: Result) {
        Glide.with(itemView.context).load("http://image.tmdb.org/t/p/w500${movie.poster_path}")
            .into(photo)
        title.text = Resources.getSystem().getString(R.string.title) + movie.title
        overview.text = movie.overview
        rating.text = Resources.getSystem().getString(R.string.ratings) + movie.vote_average.toString()
    }
